...
dtNodeEager::dtNodeEager(InstanceStream const* stream, const CategoricalAttribute a) :
  xyCount(stream->getNoValues(a), stream->getNoClasses()), 
  att(NOPARENTEAGER), 
  condProbs(instanceStream_->getNoValues(a), instanceStream_->getNoClasses()) {

    instanceStream_ = stream;
}

dtNodeEager::dtNodeEager(const CategoricalAttribute a) :
  xyCount(instanceStream_->getNoValues(a), instanceStream_->getNoClasses()), 
  att(NOPARENTEAGER), 
  condProbs(instanceStream_->getNoValues(a), instanceStream_->getNoClasses()) {
}
