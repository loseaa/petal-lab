...
itemID noOfItems = 0;
unsigned int noOfRHSvals;
std::vector<tidset> tids;
std::vector<double> alpha;
std::vector<std::string> itemNames;

std::priority_queue<rule> rules;  // the rules
...
void expandAlpha(const unsigned int depth) {
  ...
  if (depth > noOfItems) alpha.push_back(0.0);
    ...
    for (i = alpha.size(); i <= depth; i++) {
      alpha.push_back(std::min((pow(0.5, static_cast<int>(depth-1)) / (exp(log_combin(noOfItems-noOfRHSvals, depth))) * noOfRHSvals) * 0.05, alpha[depth-1]));
    }
  }
}
}