...
class aode_pw: public IncrementalLearner {
public:
	/**
	 ...
	 */
	  aode_pw(char* const *& argv, char* const * end);

	virtual ~aode_pw(void);
	...
	void getCapabilities(capabilities &c);
        ...
        /*@Naomi*/
        void compareProbability();//用于计算验证 P(Xj=Xji|Xk=Xkp,C=Cq) < P(Xj=Xji|C=Cq)

        /*@Naomi*/
        void compareH();//用于计算验证 P(Xj=Xji|Xk=Xkp,C=Cq) < P(Xj=Xji|C=Cq)
    void getWeightVector(std::vector<float> &weightVector,std::vector<bool> &active);
private:
	/**
...
//	std::vector<CategoricalAttribute> selectedAtt;
	...
	std::vector<float> mulXY;
	float sumMulXY;
	crosstab<float> cmi = NULL;
//	 std::vector<std::vector<float> > cmi;
	float sumCMI;
	int modelNum;
	std::vector<float> infoWeightGeneral;
};
