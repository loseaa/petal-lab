...
//
	...
	inline InstanceCount getCountRest(CategoricalAttribute x1, CatValue v1,
			CategoricalAttribute x2, CatValue v2, CategoricalAttribute x3,
			CatValue v3, CatValue y) const {
		return *constRefRest(x1, v1, x2, v2, x3, v3, y);
	}
	// p(x1=v1, x2=v2, x3=v3, Y=y)
...
//		assert(x1 > x2 && x2 >x3);
		...
		inline const InstanceCount *constRefRest(CategoricalAttribute x1, CatValue v1,
			CategoricalAttribute x2, CatValue v2, CategoricalAttribute x3,
			CatValue v3, CatValue y) const {
                assert(x1 > x2);
                return &xxXyCount[x1][v1 * x1 + x2][v2 * noUnSelectedCatAtts_ + x3][v3
