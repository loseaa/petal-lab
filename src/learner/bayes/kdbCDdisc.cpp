...
kdbCDdisc::kdbCDdisc(char*const*& argv, char*const* end){
  name_ = "KDB-CondDisc";
...
kdbCDdisc::kdbCDdisc(const kdbCDdisc& l)
  : discretisingStream_(l.discretisingStream_), dirTemp_(l.dirTemp_)
{
  name_ = l.name_;
  k_ = l.k_;
}

learner* kdbCDdisc::clone() const {
  return new kdbCDdisc(*this);
}

kdbCDdisc::~kdbCDdisc(void)
{
}


void  kdbCDdisc::getCapabilities(capabilities &c){
  c.setCatAtts(true);  
  c.setNumAtts(true);
}
...
void kdbCDdisc::reset(InstanceStream &is) {
  kdb::reset(is);
...
void kdbCDdisc::train(const instance &inst) {
...
void kdbCDdisc::train(InstanceStream &is){
  instance inst;
...
void kdbCDdisc::discretise(std::vector<NumValue> &vals, std::vector<CatValue> &valsDisc, std::vector<NumValue> &cuts) {
...
bool kdbCDdisc::trainingIsFinished() {
  return pass_ > 2;
}

void kdbCDdisc::classify(const instance& inst, std::vector<double> &posteriorDist) {
