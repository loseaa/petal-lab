...
//This is memory constrained (memory selective) A2DE.
//It performs two passes learning through data.
//It select attributes only for parents.
//
...
a2de_ms::a2de_ms(const a2de_ms& l) {
  name_ = l.name_;
  pass_ = 1;
  weighted = l.weighted;
  weight = l.weight;
...
learner* a2de_ms::clone() const {
  return new a2de_ms(*this);
}
...
void a2de_ms::LOOCV(const instance &inst)
{
	count++;
...
////	 scale up by maximum possible factor to reduce risk of numeric underflow
    ...
    for (CatValue fatherIndex = 1; fatherIndex < noSelectedCatAtts_; fatherIndex++) {
        ...
        const CategoricalAttribute father = order_[fatherIndex];
        const CatValue fatherVal = inst.getCatVal(father);
        XXYSubDist xxySubDist(xxxyDist_.getXXYSubDist(fatherIndex, fatherVal), noClasses_);
        XXYSubDist xxySubDistRest(xxxyDist_.getXXYSubDistRest(fatherIndex, fatherVal), noClasses_);

        for (CatValue motherIndex = 0; motherIndex < fatherIndex; motherIndex++) {

            const CategoricalAttribute mother = order_[motherIndex];
            const CatValue motherVal = inst.getCatVal(mother);
            XYSubDist xySubDist(xxySubDist.getXYSubDist(motherIndex, motherVal), noClasses_);
            XYSubDist xySubDistRest(xxySubDistRest.getXYSubDist(motherIndex, motherVal, noUnSelectedCatAtts_), noClasses_);
            ...
            if (activeParent[fatherIndex][motherIndex] == true) {
                ...
                for (CategoricalAttribute childIndex = 0; childIndex <noCatAtts_; childIndex++) {
                    ...
                    for (CatValue y = 0; y < noClasses_; y++) {
                        ...
                        {
                            if (childIndex != fatherIndex && childIndex != motherIndex) {
                            //    InstanceCount parentChildYCount =xySubDist.getCount(child, childVal, y);
                                InstanceCount parentChildYCount ;
                                if(childIndex<noSelectedCatAtts_)
                                    parentChildYCount =
                                    xxxyDist_.getCount(childIndex,childVal, fatherIndex,fatherVal, motherIndex,motherVal, y);
                                else
                                    parentChildYCount=
                                    xxxyDist_.getCountRest( fatherIndex,fatherVal, motherIndex,motherVal, childIndex-noSelectedCatAtts_,childVal,y);
								if(y==trueClass)
									parentChildYCount--;
	...
	if (pass_ == 1) {
		...
		if (mi_ == true || su_ == true || acmi_ == true ||random_ == true
			...
			if (factored_ == true)
               ...
               sumAll = instanceStream_->getNoValues(0);
                ...
                if(newAttEst_==true)
                {
                   ///accurate computation of number of selected attributes

                    /// memory of selective A2DE equal to memory of AODE
                    /// k *[C(s,3)*v'^3+C(s,2)*v'^2*(a-s)*r]=k*C(a,2)*v^2
                    ...
                    /// simplification:  s*(s-1)*v'^2*[(s-2)*v'+3(a-s)*r]=3a(a-1)*v^2
               ...
               //     sumSelected+=instanceStream_->getNoValues(order_[2]);
                    for (i =2; i < noCatAtts_;	i++) {
                        v_prime=sumSelected/i;
                        r=(sumAll-sumSelected)/(noCatAtts_-i);

                        if( i*(i-1)*pow(static_cast<double>(v_prime),2)*((i-2)*v_prime+3*(noCatAtts_-i)*r) >=3*noCatAtts_*(noCatAtts_-1)*pow(static_cast<double>(v),2) )
                            break;
	...
	else if(pass_==3)
		{
		    if(loo_==true)
			{
					...
					for (CategoricalAttribute child = 0; child < noCatAtts_;
						...
						squaredError_[parent][child] = sqrt(
								squaredError_[parent][child]
										/ (xxxyDist_.xxyCounts.xyCounts.count-1));
						if(verbosity>=3)
				...
				if(verbosity>=2)
				{
					printf("Parent pairs: %d, selected: %d\n",parentNumber,optParentIndex_);
					printf("Child: %d, selected: %d\n",noCatAtts_,optChildIndex_);
				}
			}
